...
var ShapeToolboxModule = function (map, options) {
    ...
    function _MapMouseDownHandler(e) {
        ...
        switch (_options.shapeType.toLowerCase()) {
            case 'pushpin':
                _options.targetLayer.push(new _MM.Pushpin(_currentLocation, { draggable: true }));
                _setMouseCursor("crosshair");
                break;
            case 'polygon':
            case 'polyline':
    ...
    function _Dispose() {
        if (_shape && (_shape.toString() == '[Polygon]' || _editShape.toString() == '[AdvancedShapes.Polygon]')) {
        ...
        _shape = null;
        _setMouseCursor("url(images/grab.cur)");
    ...
    //TODO - Enable switching between shapes when editing.
