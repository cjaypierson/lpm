...
var ipMapFunctions = (function(app) {
    ...
    function buildResultRowTable(index, el)
    {
        ...
        var pid     = jQuery('<td class="hidden-phone">'+((el.mls_id) ? el.mls_id : '--')+'</td>');
        var price   = jQuery('<td>'+el.price+'</td>').formatCurrency(mapOptions.currencyFormat);
    ...
    function buildResultRowOverview(index, el, rowOffset)
    {
        ...
        var price   = jQuery('<h4 class="ip-overview-price pull-right">'+el.price+'</h4>').formatCurrency(mapOptions.currencyFormat);
        var previewpop = false;
        ...
        if (el.locstate) contentstring += ', '+el.locstate;
        if (el.province) contentstring += ', '+el.province;
        if (el.country) contentstring += ', '+el.country;
        contentstring += '<br /><em>';
