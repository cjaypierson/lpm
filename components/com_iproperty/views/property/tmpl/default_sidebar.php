...
$sidecol = '<div class="well">';        
    ...
    $sidecol .= '<div class="ip-sidecol ip-mainaddress">'.$this->property_full_address.'</div>';
    ...
    if ($this->p->mls_id)   $sidecol .= '<div class="ip-sidecol"><b>'.JText::_('Neighborhood' ).'</b>: '. $this->p->mls_id.'</div>';
    if ($this->p->price2)   $sidecol .= '<div class="ip-sidecol"><b>'.JText::_('Application Fee' ).'</b>: '. $this->p->price2.'</div>';
    if ($this->p->stype_freq)   $sidecol .= '<div class="ip-sidecol"><b>'.JText::_('Security Deposit' ).'</b>: '. $this->p->stype_freq.'</div>';
    ...
    foreach ($this->agents as $a)
    {
        ...
        $sidecol .= '<div class="well">';
            ...
            if ($this->settings->agent_show_social)
            {
                ...
                if($a->skype) 
                    $sidecol .= '<div class="ip-sidecol sidecol-skype"><b><abbr title="'.JText::_('COM_IPROPERTY_SKYPE' ).'">S:</abbr></b> '.$a->skype.'</div>';
                if($a->gtalk) 
...
if ($this->openhouses)
{
    ...
    foreach ($this->openhouses as $o)
    {
        ...
        $tipstart   = ($o->comments) ? '<span class="hasTooltip" title="'.JText::_('COM_IPROPERTY_OPENHOUSE' ).'::'.htmlentities($o->comments).'">' : '';
        ...
        $sidecol    .= '
            ...
            <div class="ip-sidecol oh-details">
                <div class="ip-sidecol oh-date-start alert alert-success small"><b>Start</b><br />'.JFactory::getDate($o->startdate)->format(JText::_('COM_IPROPERTY_DATE_FORMAT_IPOH')).'</div>
                <div class="ip-sidecol oh-date-enda alert alert-error small"><b>End</b><br />'.JFactory::getDate($o->enddate)->format(JText::_('COM_IPROPERTY_DATE_FORMAT_IPOH')).'</div>
            </div>';
    }
    $sidecol .= '</div>';
}
