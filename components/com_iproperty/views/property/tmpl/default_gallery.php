...
if(!$this->images){ // set thumbnail nopic image to create tab dimensions for map and streetview, etc
    ...
    switch($this->settings->gallerytype)
    {
        case 1: // LIGHTBOX
            ...
            $this->document->addScript($this->ipbaseurl.'/components/com_iproperty/assets/galleries/lightbox/js/lightbox.js');
            // html
            echo '
            <div id="ip-image-tab">';
                $i = 1;
                foreach($this->images as $image)
                {
                    ...
                    echo '
                        <a href="'.$path.$image->fname.$image->type.'" rel="lightbox[ip-image-tab]" title="'.$image->title.'">
