...
class IpropertyModelIPuser extends JModelLegacy
{
	...
	public function getIpProperties()
	{
		...
		if( empty($this->_properties)){
            ...
            $query->select('s.*, p.id as id, p.price, p.price2, p.stype, p.stype_freq, p.call_for_price, p.title AS title, p.street_num, p.street AS street, p.street2, p.city, p.state, p.hide_address, p.sqft, p.approved, s.id AS save_id, s.timestamp AS created, p.alias as prop_alias, p.mls_id')
